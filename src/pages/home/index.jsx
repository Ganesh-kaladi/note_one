        ...
        <div className="basis-4/5 w-full">
          <div className="flex justify-center pt-6">
            <div className="flex flex-col w-[300px] border  relative rounded-md">
              ...
              <button
                disabled={
                  (text?.length <= 0 || title?.length <= 0) && "disabled"
                }
                className="absolute bottom-0 right-0 w-7 h-7  border bg-indigo-800 text-slate-50 rounded-full"
                onClick={onAddClick}
              >
          ...
          <div>
            {pinnedNote?.length > 0 && (
              <h1 className="mt-10 mb-2 pl-4">PinnedNote</h1>
            )}
            <div className="mt-18 flex flex-wrap gap-6">
              {pinnedNote.length > 0 &&
                pinnedNote.map(({ id, title, text, isPin }) => {
                  return (
                    <Notecard
                      key={id}
                      id={id}
          ...
          <div className="mt-14 flex flex-wrap gap-6">
            {otherNote.length > 0 &&
              otherNote.map(({ id, title, text, isPin }) => {
                return (
                  <Notecard
                    key={id}
                    id={id}
